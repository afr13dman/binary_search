...
def count_repeats(xs, x):
    ...
    def top_index(left, right):
        ...
        mid = (left + right) // 2
        if xs[mid] == (x - 1) and xs[mid - 1] != xs[mid]:
            return mid
        if xs[mid] < x:
            right = mid
        if xs[mid] >= x:
            left = mid + 1
        return top_index(left, right)

    def bottom_index(left, right):
        ''' Returns the lowest index with a value >= x '''
        if left + 1 == right:
            return 0
        ...
        if xs[mid] <= x:
            right = mid + 1
        return bottom_index(left, right)

    if xs[-1] == x:
        t_index = len(xs)
    else:
        t_index = top_index(0, len(xs) - 1)
    if xs[0] == x:
        b_index = -1
    else:
        b_index = bottom_index(0, len(xs) - 1)

    if t_index == b_index == 0:
        return 0
    return t_index - b_index - 1
...
def argmin(f, lo, hi, epsilon=1e-3):
    ...
    min_value = min(f(lo), f(hi), f(m1), f(m2))

    if min_value == f(lo) or min_value == f(m1):
